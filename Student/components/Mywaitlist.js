function MyWaitlist({ currentUser, setActive }) {
  ...
  React.useEffect(() => {
    fetch(
      `https://bracu-bus-portal.onrender.com/api/bookings/waitlist/user/${currentUser.studentId}`,
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  async function handleLeaveWaitlist(id) {
    ...
    try {
      const res = await fetch(
        `https://bracu-bus-portal.onrender.com/api/bookings/waitlist/${id}`,
        { method: "DELETE" },
      );
            ...
            return (
              ...
              >
                ...
                {e.status === "promoted" && (
                  ...
                  >
                    🎉 A seat opened up and you were automatically booked! Check{" "}
                    <strong>My Bookings</strong> for your ticket.
                  </div>
                )}
